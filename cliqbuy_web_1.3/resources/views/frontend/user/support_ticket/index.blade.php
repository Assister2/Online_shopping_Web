...
<div class="cls_bread">
    <ul>
       <li><a href="{{ route('mainmenu') }}">{{ translate('main') }}</a></li>
        <li><a>{{ translate('Support Ticket') }}</a></li>
    </ul>
</div>
        ...
        <div class="modal-content">
            <div class="modal-header">
              ...
              <div class="modal-body px-3 pt-3">
                  <form class="support_ticket_form" action="{{ route('support_ticket.store') }}" method="POST" enctype="multipart/form-data" id="support_ticket_form">
                      @csrf
                      <div class="row">
                          <div class="col-md-2">
                      ...
                      <div class="text-right mt-4">
                          ...
                          <button type="submit" class="btn btn-primary ticket-save">{{ translate('Send Ticket')}}</button>
                      </div>
                  </form>
              </div>
            </div>
...
<script type="text/javascript">
 /* $(document).on('click', '.ticket-save', function(){
      $(this).attr('disabled', 'disabled');
      $('.support_ticket_form').submit();
  });*/

jQuery.validator.addMethod("noSpace", function(value, element) { 
  console.log('check ',value.indexOf(" ") );
  console.log('check value ',value);
  let mergev = value.indexOf(" ") < 0 && value != "";

  console.log('mergev ',mergev);
  return value == '' || value.trim().length != 0; 
}, no_space_please);

$(document).ready(function() {
  $("#support_ticket_form").validate({
   rules: {
      subject: {
          required: true,
          noSpace: true
  ...
  $("#support_ticket_form").submit(function (event) {
        var $valid = $('#support_ticket_form').valid();
