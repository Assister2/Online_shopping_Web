...
<div class="cls_bread">
    ...
    <div class="row gutters-10 justify-content-center">
        ...
        @if($user_subscription && get_setting('subscription')=='1')
        <div class="col-md-4 mx-auto mb-3" >
              ...
              <div class="p-3 rounded mb-3 c-pointer text-center bg-white shadow-sm hov-shadow-lg has-transition">
                  ...
                  <div class="fs-18 text-primary">{{ translate('Published products') }}</div>
                  <div>{{$user_subscription->name.' '.translate('Plan').': '.$user_subscription->no_of_product.' '.translate('Products') }}</div>
                  <div><a href="{{route('seller.subscription')}}">{{ translate('Upgrade your plan') }}</a> {{ translate('to publish more products') }}</div>
              </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input onchange="update_published(this)" class='product_publish_{{$product->id}}' value="{{ $product->id }}" type="checkbox" <?php if($product->published == 1) echo "checked";?> >
                            ...
                            <td class="text-right">
		                      <a class="btn btn-soft-info btn-icon btn-circle btn-sm fs-16" href="{{route('seller.products.edit', ['id'=>$product->id, 'lang'=>env('DEFAULT_LANGUAGE')])}}" title="{{ translate('Edit') }}">
		                          <i class="las la-edit"></i>
		                      </a>
                              <a href="{{route('products.duplicate', $product->id)}}" class="btn btn-soft-success btn-icon btn-circle btn-sm fs-16"  title="{{ translate('Duplicate') }}">
    							   <i class="las la-copy"></i>
    						  </a>
                              <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete fs-16" data-href="{{route('products.destroy', $product->id)}}" title="{{ translate('Delete') }}">
        ...
        function update_featured(el){
            ...
            $.post('{{ route('products.seller.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    ...
                    AIZ.plugins.notify('danger', '{{ translate("your_are_not_verified_by_admin") }}');
                }
                else if(data == 3){
                    el.checked = false;
                    AIZ.plugins.notify('danger', '{{ translates('kindly_pay_owe_amount') }}');
                }                
                else{
        ...
        function update_published(el){
            ...
            $.post('{{ route('products.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    ...
                    $('.product_publish_'+el.value).prop('checked', false); 
                    AIZ.plugins.notify('danger', '{{ translate('Kindly upgrade the subscription plan') }}');
                }
                else if(data == 8){
                    $('.product_publish_'+el.value).prop('checked', false);
                    AIZ.plugins.notify('danger', '{{ translate("You are not verified") }}');
                }
                else if(data == 3){
                    el.checked = false;
                    AIZ.plugins.notify('danger', '{{ translates('kindly_pay_owe_amount') }}');
