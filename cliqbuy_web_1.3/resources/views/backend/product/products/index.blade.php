...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-auto">
            @if($type == 'Seller')
                <h1 class="h3">{{translate('Merchant products')}}</h1>
            @endif
            @if($type == 'All')
                <h1 class="h3">{{translate('All products')}}</h1>
            @endif
            @if($type == 'In House')
                <h1 class="h3">{{translate('In House products')}}</h1>
            @endif
        </div>
        @if($type != 'Seller')
...
<div class="card">
    <form class="" id="sort_products" action="" method="GET">
        <div class="card-header row gutters-5">
            <div class="col">
                @if($type == 'Seller')
                    <h5 class="mb-md-0 h6">{{translate('Merchant products')}}</h5>
                @endif
                @if($type == 'All')
                    <h5 class="mb-md-0 h6">{{ translate('All Product') }}</h5>
                @endif
                @if($type == 'In House')
                    <h5 class="mb-md-0 h6">{{translate('In House products')}}</h5>
                @endif
            </div>
            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id" name="user_id" onchange="sort_products()">
                    <option value="">{{ translate('All Merchants') }}</option>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="row gutters-5 w-200px w-md-300px mw-100">
                                <div class="col-auto">
                                    ...
                                    @else
                                        <img src="{{ static_asset('assets/img/placeholder.jpg') }}" height="40" style="display:inline-block;">
                                    @endif
                                </div>
                                <div class="col">
                        ...
                        <td>
                            ...
                            <strong>{{translate('Base Price')}}:</strong> {{ format_price($product->unit_price) }} </br>
                        ...
                        @php
                            $seller = App\Seller::where('user_id',$product->user->id)->first();
                            $verification_check = $seller ? $seller->verification_status == 1 : false;
                        @endphp
                        @if($verification_check || $product->user->user_type == 'admin')
                            <td>
                        ...
                        @else    
                            <td>
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input  type="checkbox" disabled="disabled" <?php if ($product->todays_deal == 1) echo "checked"; ?> >
                                    <span class="slider round"></span>
                                </label>
                            </td>
                            <td>
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input disabled="disabled" type="checkbox" <?php if ($product->published == 1) echo "checked"; ?> >
                                    <span class="slider round"></span>
                                </label>
                            </td>
                        @endif    
                        ...
                        @else
                            <td>
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input disabled="disabled" type="checkbox" <?php if ($product->featured == 1) echo "checked"; ?> >
                                    <span class="slider round"></span>
                                </label>
                            </td>
                        @endif    
    ...
    <script type="text/javascript">
        ...
        function update_todays_deal(el){
            ...
            $.post('{{ route('products.todays_deal') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                ...
                {
                    AIZ.plugins.notify('danger', '{{ translate('Not a verified Merchant') }}');
                }
                else if(data == 3) {
                    el.checked = false;
                    AIZ.plugins.notify('danger', '{{ translates('owe_amount_pending') }}');
                }
                else{
        ...
        function update_published(el){
            ...
            $.post('{{ route('products.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 7){
                    el.checked = false;
                    AIZ.plugins.notify('danger', '{{ translate('Mercant is banned') }}');
                }
                else if(data == 8){
                    el.checked = false;
                    AIZ.plugins.notify('danger', '{{ translate('Mercant is n\'t verified') }}');
                }
                else if(data == 1){
                    //el.checked = false;
                    ...
                    el.checked = false;
                    AIZ.plugins.notify('danger', '{{ translate('Merchant created products more than number of products specified in subscription which subscribed') }}');
                }
                else if(data == 3) {
                    el.checked = false;
                    AIZ.plugins.notify('danger', '{{ translates('owe_amount_pending') }}');
