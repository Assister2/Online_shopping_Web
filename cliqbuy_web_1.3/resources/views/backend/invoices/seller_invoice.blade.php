...
<body>
	<div>
		...
		<div style="background: #eceff4;padding: 1rem;">
			...
			<table>
				<tr>
					<td style="font-size: 1rem;" class="strong">{{ ($shop->name) ? $shop->name : '' }}</td>
					<td class="text-right"></td>
				</tr>
				<tr>
					<td class="gry-color small">{{ ($shop->address) ? $shop->address : '' }}</td>
					<td class="text-right"></td>
				</tr>
				<tr>
					<td class="gry-color small">{{  translate('Email') }}: {{ ($shop->email) ? $shop->email : $user->email }}</td>
					<td class="text-right small"><span class="gry-color small">{{  translate('Order ID') }}:</span> <span class="strong">{{ $order->code }}</span></td>
				</tr>
				<tr>
					<td class="gry-color small">{{  translate('Phone') }}: {{ ($shop->phone) ? $shop->phone : '' }}</td>
		...
		<div style="padding: 1rem;padding-bottom: 0">
            <table>
				...
				<tr><td class="strong">{{ ($order->user->deleted_at == '') ? $shipping_address->name : 'Deleted User' }}</td></tr>
				@if ($order->orderDetails->where('seller_id', Auth::user()->id)->where('shipping_type','!=','pickup_point')->first())
				<tr><td class="gry-color small">{{ $shipping_address->address }}, {{ $shipping_address->city }}, {{ @$shipping_address->state??'' }}, {{ $shipping_address->country }}</td></tr>
				@endif
				@if(isLiveEnv())
					<tr><td class="gry-color small">{{ translate('Email') }}: {{ protectedString($shipping_address->email) }}</td></tr>
					<tr><td class="gry-color small">{{ translate('Phone') }}: {{ protectedString($shipping_address->phone) }}</td></tr>
				@else
				...
				<tbody class="strong">
					@php
						if(request()->has('order_detail_id')) {
							$seller_orders = \App\OrderDetail::with('product')->where(['order_id' => $order->id, 'seller_id' => request()->order_detail_id])->get();
						} else {
							$seller_orders = \App\OrderDetail::with('product')->where('order_id',$order->id)->get();
						}
					@endphp
	                @foreach ($seller_orders as $key => $orderDetail)
					        ...
					        <table class="text-right sm-padding small strong">
						        <tbody>
							        <tr>
							            ...
							            <td class="currency">{{ single_price($order->orderDetails->where('seller_id', Auth::user()->id)->sum('price')) }}</td>
							        </tr>
							        <tr>
							        ...
							        <tr>
							            ...
							            <td class="currency">{{ single_price($order->orderDetails->where('order_id', $order->id)->sum('price') + $order->orderDetails->sum('tax') + $order->orderDetails->sum('shipping_cost')) }}</td>
							        </tr>
						        </tbody>
						    </table>
			            </td>
