...
trait Translatable
{
    ...
    public function getTranslationById($locale, $id = null)
    {
        $translation = $this->translations->where('id', intval($id))->first();
        if(!$translation) 
        {
    ...
    public function deleteTranslationById($id =null) 
    {
        $translation = $this->translations->where('id', intval($id))->first();
        if($translation)
        {
    ...
    {
        return get_class($this).config('translatable.translation_suffix', 'Translations');
    }
    ...
    public function inactive_translations()
    {
        return $this->hasMany($this->getTranslationModelName(), $this->getRelationKey())
        ->whereHas('language', function($query) {
    ...
    public function setAttribute($key, $value)
    {
        ...
        if ($this->isTranslationAttribute($attribute) && $locale != 'en') {
     ...
     * @return bool
     */
    public function save_dis(array $options = [])
    {
        if ($this->exists) {
    ...
    private function getFallbackLocale($locale = null)
    {
        ...
        return config('translatable.fallback_locale', 'en');
    }
    ...
    private function useFallback()
    {
        ...
        return config('translatable.use_fallback', true);
    }
    ...
    protected function getLocales()
    {
        ...
        $localesConfig = Language::where('status', '=', 'Active')->pluck('code');
    ...
    protected function saveTranslations()
    {
        $saved = true;
        foreach ($this->translations as $translation) {
            if ($saved && $this->isTranslationDirty($translation) && $translation->locale != 'en') {
    ...
    /**
     * Set the Fallback mode on the model.
     *
     * @param $locale
     *
     * @return $this
     */
    public function setFallback($useTranslationFallback = true)
    {
