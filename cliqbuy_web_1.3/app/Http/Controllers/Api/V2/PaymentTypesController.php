...
class PaymentTypesController
{
    ...
    public function getList(Request $request)
    {
        ...
        if ( get_setting('paypal_payment') == 1) {
            ...
            $payment_type['title'] = trans('messages.api.checkout_paypal');
            if($mode == 'wallet') {
        ...
        if (get_setting('stripe_payment') == 1) {
            ...
            $payment_type['title'] = trans('messages.api.checkout_stripe');
            if($mode == 'wallet') {
        ...
        if($mode != 'wallet') {
            ...
            if ( get_setting('cash_payment') == 1 && $digital != 1 && $cod_on == 1) {
                ...
                $payment_type['title'] = trans('messages.api.cash_on_delivery');

                if($request->has('user_id') && $request->user_id != '') {
                    $product_ids = Cart::select('product_id')->where('user_id', $request->user_id)->get();
                    $pdts = Product::where('cash_on_delivery','!=',1)->whereIn('id', $product_ids)->count();
                    if($pdts == 0) {
                        $payment_types[] = $payment_type;
                    }
                } else {
