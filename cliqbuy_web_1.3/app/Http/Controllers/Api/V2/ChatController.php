...
class ChatController extends Controller
{
    ...
    {
        $messages = Message::where('user_id', $request->user_id)->where('product_id',$request->product_id)->orderBy('created_at', 'desc')->paginate(10);
        return new MessageCollection($messages);
    }
    ...
    public function send_message_to_seller($conversation, $message, $seller_user, $user)
    {
        ...
        $array['content'] = trans('messages.api.you_received_msg_from') . $user->name . '.';
