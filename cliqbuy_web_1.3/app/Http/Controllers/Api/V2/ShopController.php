...
class ShopController extends Controller
{
    ...
    public function topSellingProducts($id)
    {
        if ($id == '0') 
        { 
            return new ProductMiniCollection(Product::where('added_by', 'admin')->where('published',1)->orderBy('num_of_sale', 'desc')->limit(10)->get());
        }
        else
        {
            $shop = Shop::findOrFail($id);
            return new ProductMiniCollection(Product::where('user_id', $shop->user_id)
                ->where('num_of_sale','!=',0)->where('num_of_sale','!=',null)->where('published',1)->orderBy('num_of_sale', 'desc')->limit(10)->get());
        }
    }
