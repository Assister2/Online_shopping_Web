...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        if(!$request->isMethod('POST')) {
            flash(translate('please_proceed_from_cart_page'))->warning();
        ...
        if ($request->payment_option != null) {
            ...
            foreach($carts as $cart){
                if($cart->user_id==$cart->owner_id){
                    flash(translate('dont_buy_own'))->error();
                    return redirect()->route('cart');
            ...
            if ($request->session()->get('cart_id') != null) {
                if ($request->payment_option == 'paypal') {
                    ...
                    if (\App\Addon::where('unique_identifier', 'otp_system')->first() != null &&
                        ...
                        !Auth::user()->email) {
                        flash(translate('verify_email_before_order'))->warning();
                    ...
                    if (Auth::user()->phone == null) {
                        flash('please_add_phone')->warning();
                    ...
                    flash(translate("your_order_has_been_placed_successfully"))->success();
                    return redirect()->route('order_confirmed');
                    ...
                    flash(translate('payment_info'))->success();
                    return redirect()->route('order_confirmed');
                }
            }
        } else {
            flash(translate('select_a_payment_option'))->warning();
            return back();
        }
    }
    ...
    public function checkout_done($order_id, $payment)
    {
        ...
        $orderController->checkout_store($payment_option);

        $orders = Order::whereIn('id', Session::get('order_id'))->get();

        foreach($orders as $order) {
            if($order->payment_status!='paid') {
                flash(translate('payment_completed'))->success();
            }
...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
        ...
        flash(translate('your_cart_is_empty'))->success();
        return back();
    }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if ($request->address_id == null) {
            flash(translate("add_shipping_add"))->warning();
            return back();
        }

        $carts = Cart::with(['product', 'user', 'user.addresses'])->where('user_id', Auth::user()->id)->get();
        ...
        foreach ($carts as $key => $cartItem) {
            ...
            $shipengine_count = UserShipEngineSettings::where('user_id', $cartItem->owner_id);
            if(get_setting('ship_engine') && $shipengine_count->count() && $cartItem->product->shipping_type == 'shipping_providers' && $cartItem->user->addresses->count()) {
                $shipengine_found = true;
            }
        }

        return $shipengine_found ? view('frontend.delivery_info_two', compact('carts')) : view('frontend.delivery_info', compact('carts'));
    }
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        $shipengine_found = false;
        foreach ($carts as $key => $cartItem) {
            $user_shipengine = UserShipEngineSettings::where('user_id', $cartItem->owner_id);
            if(get_setting('ship_engine') && $user_shipengine->count() && $cartItem->product->shipping_type == 'shipping_providers' && $cartItem->user->addresses->count()) {
                $shipengine_found = true;
                break;
            }
        }
            ...
            if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date) {
                if (CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null) {
                    ...
                    if ($coupon->type == 'cart_base') {
                        ...
                        $sum_percentage = $sum/100;

                        foreach ($carts as $key => $cartItem) {
                            $cart_id = $cartItem->id;
                            $cart_item_percentage = $cartItem->price/$sum_percentage;
                            $cart_price = ($coupon->discount/100) * $cart_item_percentage;
                            $cartItem_price = $cartItem->price - $cart_price;

                                if ($coupon->discount > $sum) {
                                    $cart_price = $coupon->discount;
                                }
                                
                            Cart::where('user_id', Auth::user()->id)->where('id',$cart_id)
                            ->update(
                                [
                                    'discount' => $cart_price,
                            ...
                            foreach ($coupon_details as $key => $coupon_detail) {
                                if ($coupon_detail->product_id == $cartItem['product_id']) {
                                    ...
                                    if ($coupon->discount_type == 'percent') {
                                        info($cartItem['price']);
                                        info('cartItem_price');
                                        info($cartItem['quantity']);
                                        info($coupon->discount);
                                    ...
                                    Cart::where('user_id', Auth::user()->id)->where('product_id',$cartItem['product_id'])
                                        ->update(
                                            [
...
//            if(Auth::user()->club_point->points >= $point) {
    ...
    {
        $orders = Order::whereIn('id', Session::get('order_id'));
        $order_clone = clone $orders;

        $orders = $orders->get();
        $order_user_id = $order_clone->pluck('user_id');

        Cart::whereIn('user_id', $order_user_id)->delete();
