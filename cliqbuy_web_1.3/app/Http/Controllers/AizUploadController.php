...
class AizUploadController extends Controller
{
        ...
        if($request->hasFile('aiz_file')){
            ...
            if(isset($type[$extension])){
                ...
                if (env('FILESYSTEM_DRIVER') == 's3') {
                    ...
                    file_get_contents($request->aiz_file->getRealPath()),
                    [
                    'visibility' => 'public',
    ...
    public function destroy(Request $request,$id)
    {
        ...
        if(auth()->user()->user_type == 'seller' && $upload->user_id != auth()->user()->id){
            flash(translate("you_dont_have_permission_to_delete"))->error();
            return back();
        }
        try{
            if(env('FILESYSTEM_DRIVER') == 's3'){
            ...
            \DB::statement("UPDATE products
                        SET
                        photos =
                            TRIM(BOTH ',' FROM REPLACE(CONCAT(',', photos, ','), ',$id,', ','))
                        WHERE
                        FIND_IN_SET({$id}, photos)");

            \DB::statement("UPDATE products
                        SET
                        thumbnail_img =
                            TRIM(BOTH ',' FROM REPLACE(CONCAT(',', thumbnail_img, ','), ',$id,', ','))
                        WHERE
                        FIND_IN_SET({$id}, thumbnail_img)");
                        
            flash(translate('file_deleted_successfully'))->success();
        }
        catch(\Exception $e){
            $upload->delete();
            flash(translate('file_deleted_successfully'))->success();
        ...
        if($request->segment(1) == 'seller' && $request->segment(2) == 'uploads') {
            return back();
        }
    ...
    public function attachment_download($id)
    {
        ...
        try{
           $file_path = public_path($project_attachment->file_name);
            ...
            flash(translate('file_doesnt_exists'))->error();
            return back();
        }
