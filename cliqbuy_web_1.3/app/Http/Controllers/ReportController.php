...
class ReportController extends Controller
{
    ...
    public function deleted_user_index(Request $request)
    {
        $data['users'] = User::onlyTrashed()->get();
        $data['user_name'] = '';
        $data['category'] = '';
        $data['user_info'] = null;
        $result = User::onlyTrashed();
        $data['results'] = $result->orderBy('id', 'desc')->paginate(10);

        if($request->user_name=='' && ($request->category=='' || $request->category=='users')){
        ...
        if($request->user_name!='' && $request->category=='users'){
            $data['user_name'] = $request->user_name;
            $data['category'] = $request->category;
            // $data['user_info'] = $result->where('id',$request->user_name)->first();
        }

        if($request->category=='orders'){
            $data['user_name'] = $request->user_name;
            $data['category'] = $request->category;
            $orders = Order::where('user_id', $request->user_name)->orderBy('id','desc');
            $data['orders'] = $orders->orderBy('id', 'desc')->paginate(10);
        } 

        if($request->category=='support_tickets'){
            $data['user_name'] = $request->user_name;
            $data['category'] = $request->category;
            $support_tickets = Ticket::with('user')->where('user_id', $request->user_name)->orderBy('id','desc');
            $data['support_tickets'] = $support_tickets->orderBy('id', 'desc')->paginate(10);
        } 

        if($request->category=='product_queries'){
            $data['user_name'] = $request->user_name;
            $data['category'] = $request->category;
            $product_queries = Conversation::where('sender_id', $request->user_name)->orderBy('id','desc');
            $data['product_queries'] = $product_queries->orderBy('id', 'desc')->paginate(10);
        } 
        return view('backend.delete_user_reports', $data);
